/**
 * Grant the new `deductionRemovals` permission to the roles that should have it.
 *
 * A new module is absent from every role document that already exists, and
 * `checkPermission` refuses what it cannot find — so until this runs nobody
 * can raise or approve a removal request. Super Admin is unaffected: it
 * bypasses the check entirely.
 *
 * Raising one is self-service — the person it would deduct from is the one
 * asking, the same as leave and office keeping — so everybody gets view and
 * create. `approve` is the real gate, decided by HR here.
 *
 *     bun src/seeds/grantDeductionRemovalsAccess.ts            # report only
 *     bun src/seeds/grantDeductionRemovalsAccess.ts --apply
 *
...
/** Who decides these. Payroll's own approvers, not office keeping's. */
...
async function main() {
  ...
  for (const r of roles) {
    ...
    const has = !!perms.deductionRemovals;
