...
import { useTeamAttendanceDaily } from "@/hooks/useAttendance";
...
import type { TeamAttendanceRow, DayViewStatus } from "@/types";

/**
 * A department head's own team, one day at a time — the same question the
 * admin day view answers, narrowed to whoever reports into a department this
 * login heads. Read-only: seeing clock-in/out is the point, not editing it —
 * a head gets here by heading a department, not by holding any attendance
 * permission, so there is no edit/delete action to gate.
 */
...
export function AttendanceTeamView() {
  ...
  const { data, isLoading, isFetching } = useTeamAttendanceDaily(date);
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((r) => {
      ...
      if (!term) return true;
      return `${r.employee.name} ${r.employee.employeeCode ?? ""} ${r.department}`.toLowerCase().includes(term);
    });
  }, [data, query.debouncedSearch, statusFilter]);

  const multiDept = new Set(rows.map((r) => r.department)).size > 1;
  ...
  const columns: DataTableColumn<TeamAttendanceRow>[] = [
    ...
    ...(multiDept ? [{ id: "department", label: "Department", render: (r: TeamAttendanceRow) => <span className="text-muted-foreground">{r.department || "—"}</span> } as DataTableColumn<TeamAttendanceRow>] : []),
    {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-card p-3 shadow-sm">
        ...
        <div className="ml-auto text-sm text-muted-foreground">
          {fmtLongDate(date)}
          {data && <span className="ml-2 text-xs">· {data.total} of your team</span>}
        </div>
      </div>
      ...
      <DataTable
        ...
        emptyText="Nobody on your team for this day."
